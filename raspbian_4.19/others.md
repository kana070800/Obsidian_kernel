커널에는 시스템 오동작을 유발할 수 있는 에러 조건을 점검해 커널 패닉을 유발하는 코드가 존재한다. 오동작을 보이기 전에 문제 발생 시점에서 커널 패닉으로 코어 덤프를 받아 원인을 해결하려는 의도이다.

what is "smp 핫 플러그인 스레드" >> in 책 1권 495p, 499p
	시스템 동작 중 각 cpu 별로 처리해야하는 동작을 처리하기 위한 스레드
		cpu on, off
		인터럽트 정리
		작업 큐 정리
		스케줄러 정리
